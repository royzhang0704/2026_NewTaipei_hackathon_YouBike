...
# ── 調度確認（計劃-調度確認.md §4／§12）────────────────────
#   候選站的台數下界。★ 為什麼是 2 不是 1：1 台的候選會讓清單塞滿零星
#   站，coverage-walk 得跑五六站才湊滿一次調度，文案也難寫（「從 A 調
#   1 台、B 調 1 台、C 調 2 台…」沒有調度意義）。同時是寫入端的驗證
#   下界 —— 前端 stepper 調到 1 會被整批 400 擋掉。
DISPATCH_MIN_BIKES = 2

#   候選演算法版本。溯源用，語意同 RISK_ALGO_VER 但**不進 DB**（調度單
#   是使用者的決定，不是可重算的衍生資料，沒有「換版重判」這回事）。
#   改候選規則（方向定義／同區優先／排序／floor 口徑）時換這個字串，
#   好讓 debug log 與交件文件對得起來。
DISPATCH_ALGO_VER = "dispatch-v1"
