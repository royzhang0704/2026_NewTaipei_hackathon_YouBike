...
// 嚴重度軸的文字：KPI / 篩選 chip 用「現在 / 預測」（hover 補完整語意）
const LV_CHIP: Record<string, string> = { high: '現在', mid: '預測' }
const LV_HINT: Record<string, string> = {
  high: '已缺車或滿站',
  mid: '1 小時內缺車或滿站',
}

// 清單列每站處境用白話敘述（見 lib/risk.ts riskPhrase）：現正缺車 / 即將滿站 / 車量偏低。
// 篩選 chip 是分類標籤「現在／預測」，這裡是敘述，「即將」與「預測」桶相容。
...
export function AlertList() {
  ...
  // 報讀者狀態訊息：篩選一變、count 一變就唸「（預測・）缺車・共 N 筆待處理」
  const scopeLabel = [LV_CHIP[level] ?? '', side === 'shortage' ? '缺車' : side === 'full' ? '滿站' : '']
    .filter(Boolean)
    .join('・')
  ...
  return (
    <>
      ...
      <div ref={headRef} className="sticky top-0 z-10 border-b border-hair bg-panel">
        {/* 兩條互斥軸：方向（全部/缺車/滿站）與狀態（現在/預測＝原高/中風險）。各自包成 role=group、
            中間一條分隔線 → 視覺與報讀者都看得出「這是兩個單選」，不會誤以為四顆可同時選。
            大字級塞不下時整組一起換到第二行（不會只有第二顆落單）。
            分頁不帶計數：數字全交給下方狀態列與上方 KPI，避免不吃狀態的徽章跟畫面對不上。 */}
        <div className="flex flex-wrap items-center gap-[6px] px-3 pt-[7px]">
          ...
          {/* 狀態：現在 / 預測 互斥 toggle（點目前選中的 → 回「不限」）。跟上方 KPI 連動同一個 store 值 */}
          <div role="group" aria-label="現況或預測" className="flex items-center gap-[6px]">
            {(['high', 'mid'] as const).map((lv) => (
              <button
                ...
                aria-label={`${LV_CHIP[lv]}：${LV_HINT[lv]}`}
                title={LV_HINT[lv]}
              ...
              <li key={it.station_uid} className="anim-row border-b border-hair">
                ...
                >
                  ...
                  <span className="min-w-0">
                    {/* 固定 3 行：站名 ／ 行政區·狀態 ／ 可借 X／Y。每段內容都短、whitespace-nowrap，
                    ...
                    <span className="mt-[2px] block whitespace-nowrap text-[0.73rem] tracking-[0.04em] text-ink3">
                      {it.town}
                      {riskPhrase(it.level, it.side) ? `　·　${riskPhrase(it.level, it.side)}` : ''}
                    </span>
