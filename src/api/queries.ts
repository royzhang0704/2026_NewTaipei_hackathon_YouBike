...
export function useHealth() {
  return useQuery({
    ...
    /* 短間隔：頭欄時鐘靠連續兩次 now 推算流速，後端調速度最多這麼久就跟上。
       ★ 現算中（predicting_origin 非 null）縮到 2s —— 全頁遮罩靠這支的回應
         關閉，間隔太長的話評審會看到「算完了畫面還鎖著」。
       ★ 2026-09-12 二修：非現算的基礎間隔 8s → 3s。後端把遮罩旗標從「只蓋
         現算」擴大到整格（搬資料＋預測＋風險＋收單）之後，一格推進可能只花
         幾秒 —— 8s 輪詢會整段錯過，遮罩等於沒開，使用者只看到畫面閃一下。
         ⚠ 代價：healthz 的請求量變成 2.7 倍。那支是幾個 sys_config 查詢，
           demo 規模吃得下；真要省請改用 SSE 推播，不要把間隔調回去。 */
    refetchInterval: (q) => (q.state.data?.predicting_origin ? 2_000 : 3_000),
    staleTime: 2_000,
  })
}
